...
./gradlew clean build
cd ..
docker compose build consumer-a --no-cache
docker compose up -d

## 2026-01-09
- consumer-a: ビルド失敗の解消（テスト依存の見直し）
- quarkus-rest-assured を外し、RestAssured に統一
- テストで SmallRye in-memory コネクタを使用できるよう設定追加
- `/quotes/hello` に合わせてテスト修正、テスト用 application.properties 追加
- `./gradlew build` が成功することを確認（テスト緑）
